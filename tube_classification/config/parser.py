...
class CameraConfig(BaseModel):
    ...
    depth_postprocess: Optional[dict] = None
    sensor_options: Optional[dict] = None
...
class BilateralConfig(BaseModel):
    enabled: bool = True
...
class GuidedFilterConfig(BaseModel):
    enabled: bool = True
    radius: int = 4
    eps: float = 0.01
...
class MaskRefinementConfig(BaseModel):
    enabled: bool = False
    depth_sigma: float = 1.0
    morpho_kernel_size: int = 3
    min_mask_area: int = 100


class PNG16ExportConfig(BaseModel):
    enabled: bool = False


class PreprocessingConfig(BaseModel):
    bilateral: BilateralConfig = BilateralConfig()
    guided_filter: GuidedFilterConfig = GuidedFilterConfig()
    ...
    mask_refinement: MaskRefinementConfig = MaskRefinementConfig()
    png16_export: PNG16ExportConfig = PNG16ExportConfig()
